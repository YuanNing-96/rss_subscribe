...
class XmlCheckTask {
    ...
    suspend fun main(){
        ...
        subscribes.forEach {
            ...
            try {
                xml = Http.getXmlFromUrl(url)
            }catch (e : Exception){
                logger.error(Exception("网络不畅或url格式错误，订阅${subscribe.name}的链接${url}，获取xml信息失败").toString())
                return@forEach
            }
